...
const FloatingCTAs = () => {
  ...
  const waText = encodeURIComponent('नमस्ते! मैं एक परामर्श बुक करना चाहता हूं।')

  const whatsappUrl = useMemo(() => `https://wa.me/${waNumber}?text=${waText}`, [waNumber, waText])
  const telUrl = useMemo(() => `tel:${phoneNumber}`, [phoneNumber])
  ...
  return (
    <div className="fixed bottom-4 right-4 sm:bottom-6 sm:right-6 z-40 flex flex-col gap-2 sm:gap-3">
      ...
      <a
        href={telUrl}
        className="w-12 h-12 sm:w-14 sm:h-14 bg-primary hover:bg-primary/90 rounded-full flex items-center justify-center shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-110 active:scale-95"
